    ...
    private sealed class Session
    {
        ...
        private void RetargetImcGroups(PenumbraMod result, bool dropUnrelated)
        {
            for (var i = result.Groups.Count - 1; i >= 0; i--)
            {
                ...
                else if (dropUnrelated) group.Node["__apic_drop"] = true;
            }
        }
        ...
        {
            var keep = result.Groups.Where(g => !g.Node.ContainsKey("__apic_drop") &&
            ...
            foreach (var group in result.Groups) group.Node.Remove("__apic_drop");
        }
